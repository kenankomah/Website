...
var number;
var MaxCircles = 5;
var P,Q,Px,Py,Qx,Qy,Dx,Dy,R,T,U,V,Rx,Tx,Ux,Vx,Ry,Ty,Uy,Vy,X,Y,Z,A,Xx,Yx,Zx,Ax,Xy,Yy,Zy,Ay,K;
var dotProduct;
var dotProductTwo;
var theta;
var theta2;
...
function targeting(){
	if(drawEnemy < hitCount){  //the code within the if statement will run whilst the the shooter has been hit less than hitCount number of times
		Xx = player.x - shooter[0].x1;
		Xy = player.y - shooter[0].y1; 
		
		Yx = player.x - player.x+b*Math.cos(player.phi);
		Yy = player.y - player.y-b*Math.sin(player.phi);
	 
		X = Math.sqrt(Math.pow(Xx,2) + Math.pow(Xy,2)); //X is the distance between the player and the shooter
		Y = Math.sqrt(Math.pow(Yx,2) + Math.pow(Yy,2)); //Y calculates the difference between the x or y-coordinates the centre of the player 
		
		var	dotProductThree = Xx*Yx + Xy*Yy;
   		theta3 = (180/pi)*Math.acos(dotProductThree/(X*Y));
		
		Zx = player.x-b*Math.cos(player.phi-a) - shooter[0].x1;
		Zy = player.y+b*Math.sin(player.phi-a) - shooter[0].y1; 
	
		Ax = player.x-b*Math.cos(player.phi+a) - shooter[0].x1;
		Ay = player.y+b*Math.sin(player.phi+a) - shooter[0].y1; 
		
		Z = Math.sqrt(Math.pow(Zx,2) + Math.pow(Zy,2));
		A = Math.sqrt(Math.pow(Ax,2) + Math.pow(Ay,2));
		
		if(Z > A){
	   		player.phi -= pi/100;
			if(theta3 > 179){
			 	fireShot(); //shoots when its within 1 degree of the target
			}
		}else{
			player.phi += pi/100;
			if(theta3 > 179){
		...
		for(i=0; i<particles.length; i++){
			if(particles.length>0){ // this prevents the code below from running when the array is empty
				Rx = player.x - particles[i].x2;
				Ry = player.y - particles[i].y2; 
				R = Math.sqrt(Math.pow(Rx,2) + Math.pow(Ry,2));
				
				var ETA = R/14; //calculation of the time it takes for the bullet to get to its target
				var displacementX = particles[i].xSpeed*ETA; // displacementX,Y compensates for the movement of the target
				var displacementY = particles[i].ySpeed*ETA; // in the time it takes for the bullet to reach it.
				var	RRx = player.x -(particles[i].x2 + displacementX);
				var	RRy = player.y -(particles[i].y2 + displacementY); 

				Tx = player.x - player.x+b*Math.cos(player.phi);
				Ty = player.y - player.y-b*Math.sin(player.phi);
	 
				var	RR = Math.sqrt(Math.pow(RRx,2) + Math.pow(RRy,2));
				T = Math.sqrt(Math.pow(Tx,2) + Math.pow(Ty,2));
   				dotProductTwo = RRx*Tx + RRy*Ty;
   				theta2 = (180/pi)*Math.acos(dotProductTwo/(RR*T));
				Ux = player.x-b*Math.cos(player.phi-a) - particles[i].x2-displacementX;
				Uy = player.y+b*Math.sin(player.phi-a) - particles[i].y2-displacementY;
				Vx = player.x-b*Math.cos(player.phi+a) - particles[i].x2-displacementX;
				Vy = player.y+b*Math.sin(player.phi+a) - particles[i].y2-displacementY;
				K = Math.sqrt(Math.pow(Rx,2) + Math.pow(Ry,2));
				U = Math.sqrt(Math.pow(Ux,2) + Math.pow(Uy,2));
				V = Math.sqrt(Math.pow(Vx,2) + Math.pow(Vy,2));
				if(theta2 > 179){
					if((particles[i].x2 + displacementX - particles[i].radius) > 0 && (particles[i].x2 + displacementX + particles[i].radius) < canvas.width && (particles[i].y2 + displacementY - particles[i].radius) > 0 && (particles[i].y2 + displacementY + particles[i].radius) < canvas.height){
						fireShot();
					}	
				}
			}
		//The code below allows for targetting of the last item in the array	
		var	Rx1 = player.x - particles[particles.length-1].x2;
		var	Ry1 = player.y - particles[particles.length-1].y2; 
		var	R1 = Math.sqrt(Math.pow(Rx1,2) + Math.pow(Ry1,2));
		var ETA1 = R1/14;
		var displacementX1 = particles[particles.length-1].xSpeed*ETA1;
		var displacementY1 = particles[particles.length-1].ySpeed*ETA1;
		var	RRx1 = player.x -(particles[particles.length-1].x2 + displacementX1);
		var	RRy1 = player.y - (particles[particles.length-1].y2 + displacementY1); 
		var	Tx1 = player.x - player.x+b*Math.cos(player.phi);
		var	Ty1 = player.y - player.y-b*Math.sin(player.phi);
		var	RR1 = Math.sqrt(Math.pow(RRx1,2) + Math.pow(RRy1,2));
		var	T1 = Math.sqrt(Math.pow(Tx1,2) + Math.pow(Ty1,2));
   
		dotProductTwo = RRx1*Tx1 + RRy1*Ty1;
   		theta3 = (180/pi)*Math.acos(dotProductTwo/(RR1*T1));

		var	Ux1 = player.x-b*Math.cos(player.phi-a) - particles[particles.length-1].x2-displacementX1;
		var	Uy1 = player.y+b*Math.sin(player.phi-a) - particles[particles.length-1].y2-displacementY1;
		var	Vx1 = player.x-b*Math.cos(player.phi+a) - particles[particles.length-1].x2-displacementX1;
		var	Vy1 = player.y+b*Math.sin(player.phi+a) - particles[particles.length-1].y2-displacementY1;
		var	K1 = Math.sqrt(Math.pow(Rx1,2) + Math.pow(Ry1,2));
		var	U1 = Math.sqrt(Math.pow(Ux1,2) + Math.pow(Uy1,2));
...
for(i=0; i<particles.length; i++){
	var projectionPlayerDis; //distance between player and the circles
	var projectionPlayerDis2; //distance between the front of the player and the circles
	projectionPlayerDis = Math.sqrt(Math.pow((player.x - (particles[i].x2 +lead*particles[i].xSpeed)),2) +  Math.pow((player.y - (particles[i].y2 +lead*particles[i].ySpeed)),2));
 	projectionPlayerDis2 = Math.sqrt(Math.pow((player.x - (particles[i].x2 +particles[i].radius*particles[i].xSpeed)),2) +  Math.pow((player.y - (particles[i].y2 +particles[i].radius*particles[i].ySpeed)),2))
	var circlePlayerTipDis = Math.sqrt(Math.pow((player.x + b*Math.cos(player.phi) - particles[i].x2),2) +  Math.pow((player.y - b*Math.sin(player.phi) - particles[i].y2),2));
		if(projectionPlayerDis < particles[i].radius +13){
			if(circlePlayerTipDis > particles[i].radius +12){
				forward();
			}else{
				reverse();
			}	
		}	
		if(projectionPlayerDis2 < particles[i].radius +13){
			if(circlePlayerTipDis > particles[i].radius +12){
				forward();
			}else{
				reverse();
...
function forward(){ 
	if(!stop){ 
		player.y -= 1.5*Math.sin(player.phi)*1.667; // moves the player forwards, sin and cos calculations ensures that it moves in the right direction 
		player.x += 1.5*Math.cos(player.phi)*1.667;
...
function clockwise(){
	if(!stop){ 
		player.phi -= pi/100/(particles.length);  // clockwise rotation
		if(player.y-b*Math.sin(player.phi)<0){
			player.y += wall_bounce;
		}else if(player.y-b*Math.sin(player.phi)> canvas.height){
			player.y -= wall_bounce;
...
function anticlockwise(){
	if(!stop){ 
		player.phi += pi/100/(particles.length);  
		if(player.y-b*Math.sin(player.phi)<0){
			player.y += wall_bounce;
		}else if(player.y-b*Math.sin(player.phi)> canvas.height){
			player.y -= wall_bounce;
...
function shoot(){
	if(!stop){
		if(fire.length < 1){ // allows for only three shots at a time
			fire.push(new Shots());
			playerShot.currentTime = 0;
			playerShot.play();
			numberOfShots += 1; //creates new shots
		}
	}	
}
...
/********************************************** This function is used to keep count of the number of times the player has been hit by the shooter *********************************/
function shotImpact1(){ 
	playerColour = 'blue';
	for(i=0; i<enemyFire.length; i++){	
...
/********************************************** This function is used to keep count of the number of times the player has been hit by the circle *********************************/
function circleImpact(){
    max = 0;
	sum = 0;
	hits = 0;
	count.splice(1,count.length-1); // all the elements in the array except the first one are removed 
	for(i=0; i<particles.length; i++){	
	...
	if(sum){ // sum will not be zero if there is an impact
		alpha = 0.3; // fades the player when it gets hit
	}else{
		alpha = 1;	// zero transparency when there are no hits
	}
	...
	for(i=0; i<impactCounter.length; i++){
		if(impactCounter[i] != impactCounter[i+1]){ // finds the changes in impactCounter where there is a shift from 0 to 1 or vice versa
			max += 1; 
		}
	}
...
function text(){
	...
	if(totalHits >=6){
		...
		if((score - 100*totalImpacts)>0){
			...
			score = (score - 100*totalImpacts) + bonusPoints;
		    ...
		    ctx.fillText('Game Over!',e,f);
			ctx.font = '20px Arial'; 
			ctx.fillText('Your Score Is ' + (score-100*totalImpacts),e,f+60);
			ctx.font = '17px Arial'; 
